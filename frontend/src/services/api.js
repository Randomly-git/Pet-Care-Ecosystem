...
export const communityAPI = {
  // 动态相关（通过网关路由，路径改为 /api/v1）
  getMomentsByUser: (userId) => {
    const endpoint = `/api/v1/moments/user/${userId}`
    return fetch(endpoint).then(res => res.json())
  },

  createMoment: (userId, content, mediaIds = []) => {
    const endpoint = '/api/v1/moments'
  ...
  getCommentsByMoment: (momentId) => {
    const endpoint = `/api/v1/comments/moment/${momentId}`
    return fetch(endpoint).then(res => res.json())
  },
  ...
  getFollowersCount: (userId) => {
    const endpoint = `/api/v1/follows/followers/count/${userId}`
    return fetch(endpoint).then(res => res.text()).then(text => parseInt(text))
  },

  getFollowingCount: (userId) => {
    const endpoint = `/api/v1/follows/following/count/${userId}`
    return fetch(endpoint).then(res => res.text()).then(text => parseInt(text))
  }
}

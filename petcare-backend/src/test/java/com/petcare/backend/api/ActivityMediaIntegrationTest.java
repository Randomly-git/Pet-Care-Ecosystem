...
//            testDeleteActivityRecord(recordId);
//
//            // 额外测试：独立的媒体管理接口
//            testMediaManagementApis();
//        }
    ...
    private static void testSearchActivityRecords() throws IOException, InterruptedException {
        ...
        String url = BASE_URL + "/api/activities/records/pet/" + PET_ID +
                "?startDate=" + oneWeekAgo.format(DateTimeFormatter.ISO_DATE_TIME) +
                "&endDate=" + now.format(DateTimeFormatter.ISO_DATE_TIME) +
                "&activityKindId=" + ACTIVITY_ID;
     ...
     * 测试独立的媒体管理API
     */
    private static void testMediaManagementApis() throws IOException, InterruptedException {
        System.out.println("=== 额外测试：独立的媒体管理API ===");

        // 先创建一个不带文件的活动记录
        Long testRecordId = createSimpleActivityRecord();

        if (testRecordId != null) {
            // 测试上传媒体文件
            testUploadMediaToRecord(testRecordId);

            // 测试获取媒体文件
            testGetRecordMedia(testRecordId);

            // 最后清理测试记录
            cleanupTestRecord(testRecordId);
        }
    }

    private static Long createSimpleActivityRecord() throws IOException, InterruptedException {
        System.out.println("--- 创建测试用活动记录 ---");

        String url = BASE_URL + "/api/activities/records/pet/" + PET_ID +
                "?activityId=" + ACTIVITY_ID +
                "&description=测试媒体API的记录" +
                "&userId=" + USER_ID;
        ...
        if (response.statusCode() == 200) {
            String idStr = response.body().split("\"id\":")[1].split(",")[0].trim();
            Long recordId = Long.parseLong(idStr);
            System.out.println("创建的测试记录ID: " + recordId);
            return recordId;
        }
        return null;
    }

    private static void testUploadMediaToRecord(Long recordId) throws IOException, InterruptedException {
        System.out.println("\n--- 测试独立上传媒体文件 ---");

        String boundary = UUID.randomUUID().toString();

        // 创建测试文件
        File testFile = new File("media-test-" + System.currentTimeMillis() + ".txt");
        Files.writeString(testFile.toPath(), "独立上传的媒体文件内容");
        ...
        requestBody.append("--").append(boundary).append("\r\n");
        requestBody.append("Content-Disposition: form-data; name=\"file\"; filename=\"").append(testFile.getName()).append("\"\r\n");
        requestBody.append("Content-Type: text/plain\r\n\r\n");

        byte[] textPartBytes = requestBody.toString().getBytes();
        byte[] filePartBytes = fileBytes;
        byte[] endBoundary = ("\r\n--" + boundary + "--\r\n").getBytes();

        byte[] fullBody = new byte[textPartBytes.length + filePartBytes.length + endBoundary.length];
        System.arraycopy(textPartBytes, 0, fullBody, 0, textPartBytes.length);
        System.arraycopy(filePartBytes, 0, fullBody, textPartBytes.length, filePartBytes.length);
        System.arraycopy(endBoundary, 0, fullBody, textPartBytes.length + filePartBytes.length, endBoundary.length);

        String url = BASE_URL + "/api/activities/records/" + recordId + "/media" +
                "?userId=" + USER_ID;
        ...
        System.out.println("上传媒体文件响应: " + response.statusCode());
    ...
    private static void testGetRecordMedia(Long recordId) throws IOException, InterruptedException {
        System.out.println("\n--- 测试获取媒体文件列表 ---");
        ...
        System.out.println("获取媒体文件响应: " + response.statusCode());
        System.out.println("媒体文件列表:");
        System.out.println(formatJson(response.body()));
    }

    private static void cleanupTestRecord(Long recordId) throws IOException, InterruptedException {
        System.out.println("\n--- 清理测试记录 ---");

        String url = BASE_URL + "/api/activities/records/" + recordId;
        ...
        System.out.println("删除测试记录响应: " + response.statusCode());
    }

...
import com.petcare.backend.config.RabbitMQConfig;
import com.petcare.backend.dto.message.MediaBindEvent;
...
public class MediaEventPublisher {
     ...
     * @param userId          用户ID
     */
    public void publishActivityMediaBindEvent(Long activityRecordId, List<Long> mediaIds, Long userId) {
        MediaBindEvent event = MediaBindEvent.builder()
                .eventId(UUID.randomUUID().toString())
                .eventType("ACTIVITY_RECORD")
                .businessId(activityRecordId)
                .mediaIds(mediaIds)
     ...
     * 发布状态记录媒体关联事件
     *
     * @param statusRecordId 状态记录ID
     * @param mediaIds      媒体文件ID列表
     * @param userId        用户ID
     */
    public void publishStatusMediaBindEvent(Long statusRecordId, List<Long> mediaIds, Long userId) {
        MediaBindEvent event = MediaBindEvent.builder()
                .eventId(UUID.randomUUID().toString())
                .eventType("STATUS_RECORD")
                .businessId(statusRecordId)
                .mediaIds(mediaIds)
                .relatedType("STATUS")
                .userId(userId)
                .timestamp(LocalDateTime.now())

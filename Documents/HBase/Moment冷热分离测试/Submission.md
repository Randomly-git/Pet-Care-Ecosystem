...
| 类型 | 数量 | 描述 |
|------|------|------|
| **TCI（测试覆盖项）** | **27 项** | 涵盖 STT 状态路径、DT 决策规则及 BVA 边界值 |
| **Test Cases（黑盒用例）** | **27 条** | 包含正常迁移、异常回滚及业务生命周期流转 |
| **Automated Scripts** | **18 个** | 核心业务代码覆盖率达到 85% 以上 |
...
1. **风险评估准则缺失**：AI 默认无法感知业务的重要性优先级。例如，它最初将“存储空间浪费”与“数据丢失”视为同等级别的风险。为了解决此问题，我们必须在提示词中人为强制引入评价规则（即：**数据丢失 > 数据完整性 > 数据一致性 > 存储空间浪费**）。
...
- **针对风险评估 (Refined Prompt A)**：
  > "请重新评估风险等级。请严格遵守以下权重准则：任何可能导致数据丢失的行为赋予最高权重 5，数据损坏为 4，状态不一致为 3，仅影响存储空间或非核心查询的为 1。"

- **针对概念把握与分类 (Refined Prompt B)**：
  > "不要直接输出用例。请先按‘数据一致性’、‘并发控制’、‘系统可靠性’三个维度输出质量风险矩阵，然后再基于该矩阵生成对应的 Test Suite 结构。"

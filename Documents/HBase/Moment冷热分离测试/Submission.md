...
在实践过程中，我们观察到 AI 在生成测试用例时存在以下五个核心局限：
...
基于 `风险-用例覆盖矩阵` 与 `风险追溯矩阵（RTM）` 的量化统计，对本次测试的风险覆盖与用例有效性进行如下分析。
...
- **TC-2-01（冷数据综合恢复全链路）** 覆盖了10项风险 ，属于高价值场景用例，符合“路径敏感化”设计的预期。
- 所有 **16 条测试用例** 均至少关联 1 项风险，无“零覆盖”用例，用例设计具备有效性。
---

...
自然语言对话 + 工具调用。**每次调用独立，不记忆上下文。**

用户说"记录散步" → AI 问时间 → 用户说"下午5点" → AI 问备注 → 用户说"在小区遛了30分钟" → AI 展示确认 → 用户说"执行" → 调用接口创建记录
...
| 字段 | 类型 | 说明 |
|------|------|------|
| toolName | String! | 工具名（get_pet_info / get_activity_records / create_activity_record） |
...
async function agentChat(message) {
  const query = `{ aiAgent(petId: "4", message: "${message.replace(/"/g, '\\"')}") { message toolCalls { toolName arguments success } } }`
  const res = await callGraphQL(query)
